...
- `choose_move(battle: Battle)` : C'est la méthode principale invoquée à chaque tour. Elle prend un objet `Battle` et retourne une chaîne d'action basée sur la sortie du LLM.
