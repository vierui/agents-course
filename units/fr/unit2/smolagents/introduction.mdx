...
Bienvenue dans ce module, où vous apprendrez **comment construire des agents efficaces** en utilisant la bibliothèque [`smolagents`](https://github.com/huggingface/smolagents), qui fournit un *framework* léger pour créer des agents IA performants.  

`smolagents` est une bibliothèque Hugging Face ; par conséquent, nous apprécierions votre soutien en **mettant une étoile** au [`dépôt`](https://github.com/huggingface/smolagents) smolagents :
...
Nous explorerons des types d'agents critiques, y compris les agents de code conçus pour les tâches de développement logiciel, les agents d'appel d'outils pour créer des flux de travail modulaires basés sur des fonctions, et les agents de récupération qui accèdent et synthétisent l'information. 
...
Dans cette unité, Alfred, l'agent de l'unité 1, fait son retour. Cette fois, il utilise le *framework* `smolagents` pour son fonctionnement interne. Ensemble, nous explorerons les concepts clés derrière ce *framework* alors qu'Alfred s'attaque à diverses tâches. Alfred organise une fête au manoir Wayne pendant que la famille Wayne 🦇 est absente, et il a beaucoup à faire. Rejoignez-nous pour découvrir son parcours et comment il gère ces tâches avec `smolagents` !

<Tip>

Dans cette unité, vous apprendrez à construire des agents IA avec la bibliothèque `smolagents`. Vos agents pourront rechercher des données, exécuter du code et interagir avec des pages web. Vous apprendrez également comment combiner plusieurs agents pour créer des systèmes plus puissants.
...
Pendant cette unité sur `smolagents`, nous couvrons :  
...
Comme nous l'avons vu dans l'unité 1, les outils sont des fonctions qu'un *LLM* peut utiliser dans un système agentique, et ils agissent comme les blocs de construction essentiels pour le comportement de l'agent. Ce module couvre comment créer des outils, leur structure et différentes méthodes d'implémentation utilisant la classe `Tool` ou le décorateur `@tool`. Vous apprendrez également sur la boîte à outils par défaut, comment partager des outils avec la communauté et comment charger des outils contribués par la communauté pour les utiliser dans vos agents.

### 5️⃣ [Agents de récupération](./retrieval_agents)

Les agents de récupération permettent aux modèles d'accéder aux bases de connaissances, rendant possible la recherche, la synthèse et la récupération d'informations à partir de plusieurs sources. Ils exploitent des magasins de vecteurs pour une récupération efficace et implémentent des modèles de **génération augmentée par récupération (*RAG*)**. Ces agents sont particulièrement utiles pour intégrer la recherche web avec des bases de connaissances personnalisées tout en maintenant le contexte de conversation à travers des systèmes de mémoire. Ce module explore les stratégies d'implémentation, y compris les mécanismes de repli pour une récupération d'information robuste.
...
Les agents de vision étendent les capacités traditionnelles des agents en incorporant des **modèles de vision-langage (*VLMs*)**, leur permettant de traiter et d'interpréter des informations visuelles. Ce module explore comment concevoir et intégrer des agents alimentés par *VLM*, débloquant des fonctionnalités avancées comme le raisonnement basé sur l'image, l'analyse de données visuelles et les interactions multimodales. Nous utiliserons également des agents de vision pour construire un agent de navigation qui peut parcourir le web et en extraire des informations.
...
- [Guide d'appel de fonctions](https://platform.openai.com/docs/guides/function-calling) - Comprendre l'appel de fonctions dans les *LLMs*
