# Création d'un outil RAG pour les histoires d'invités
...
3. Alfred doit récupérer des détails précis comme les adresses *e-mail*
...
Pour une référence pratique, consultez [ce *HF Space*](https://huggingface.co/spaces/agents-course/Unit_3_Agentic_RAG), où le *RAG agentique* développé dans cette unité est en direct. N'hésitez pas à le cloner et à expérimenter !
...
## Construction de l'outil livre d'or
...
3. Intégrer l'outil avec Alfred
...
Nous utiliserons la bibliothèque `datasets` de *Hugging Face* pour charger le jeu de données et le convertir en une liste d'objets `Document` du module `llama_index.core.schema`.
...
Enfin, assemblons tout en créant notre agent et en l'équipant de notre outil personnalisé :
...
```
🎩 Réponse d'Alfred :
Lady Ada Lovelace est une mathématicienne estimée et amie, renommée pour son travail pionnier en mathématiques et en informatique. Elle est célébrée comme la première programmeuse informatique en raison de son travail sur la machine analytique de Charles Babbage. Son e-mail est ada.lovelace@example.com.
```
...
1. **Améliorer le récupérateur** pour utiliser un algorithme plus sophistiqué comme [sentence-transformers](https://www.sbert.net/)
...
Quand vous avez terminé, implémentez votre outil de récupération d'invités dans le fichier <code>retriever.py</code>.
</Tip>